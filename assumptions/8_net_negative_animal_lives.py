...
living. This assumption does two things:
  ...
  1. it CHANGES A PARAMETER — redefines `farmed_suffering_per_life_year()` (the
     hook the meat-eater problem reads) to a much larger value, so IF this chain
     also holds the meat-eater problem, creating a farmed life is a severe harm
     and human charities are driven firmly net-negative; and
...
# by domain — largest for the vast wild-invertebrate populations.
...
def farmed_suffering_per_life_year():
    ...
    return _NET_NEGATIVE_SUFFERING_PER_LIFE_YEAR
